...
const SavedDestinations: React.FC<SavedDestinationsProps> = ({ userId }) => {
  const [savedDestinations, setSavedDestinations] = useState<SavedVisited[]>([]);
  ...
  const loadSavedDestinations = async (page: number = 0) => {
    try {
      setLoading(true);
      setError(null);

      const destinations = await getSavedDestinations(
        userId, 
      ...
      if (page === 0) {
        ...
        setSavedDestinations(prev => [...prev, ...destinations]);
      }
  ...
  const handleLoadMore = () => {
    ...
    loadSavedDestinations(nextPage);
  };

  if (loading && savedDestinations.length === 0) {
    return (
      <div className="card">
    ...
    return (
      <div className="card">
        ...
        <div className="text-center py-8">
          ...
          <button
            onClick={() => loadSavedDestinations(0)}
      ...
      {savedDestinations.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-gray-500 mb-4">
            Start exploring destinations and save the ones you're interested in!
          </p>
        ...
        <>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {savedDestinations.map((savedDest) => {
              const destination = savedDest.destination;
